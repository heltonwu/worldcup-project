...
echo "$($PSQL "SELECT DISTINCT name FROM teams INNER JOIN games ON games.winner_id=teams.team_id WHERE year=2014 AND round='Eighth-Final' UNION SELECT name  FROM teams INNER JOIN games ON teams.team_id = games.opponent_id WHERE year=2014 AND round='Eighth-Final' ORDER BY name")"
